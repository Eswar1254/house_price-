...
def predict(bed, bath, loc, size, facing, Type):
    ...
    input_data = np.array([[bed, bath, selected_location,
                            ...
                            selected_property]], dtype=float)  # Ensure data type is consistent
...
if __name__ == "__main__":
    ...
    result = predict(bed, bath, loc, size, facing, Type)[0]  # Ensure to access the first element
    submit_button = st.button("Submit")
    if submit_button:
        larger_text = f"<h2 style='color: blue;'>The Predicted House Price is : {result:.2f} Lakhs</h2>"
